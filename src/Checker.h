...
public:
	...
	Checker(shared_ptr<Context> ctx);
	virtual ~Checker();
...
private:
	...
	bool isAssignable(shared_ptr<Type> left, shared_ptr<Type> right);

	void printAssignError(Location loc, shared_ptr<Type> left, shared_ptr<Type> right);
};
